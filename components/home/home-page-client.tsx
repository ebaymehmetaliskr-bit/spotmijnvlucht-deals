...
export default function HomePageClient({ initialDeals, initialError, fallbackUsed }: HomePageClientProps) {
  const [deals, setDeals] = useState<NormalizedDeal[]>(() => (initialDeals.length > 0 ? initialDeals : sampleDeals))
  const [error, setError] = useState<string | null>(initialError ?? null)
  const [usedFallback, setUsedFallback] = useState<boolean>(() =>
    initialDeals.length === 0 ? true : Boolean(fallbackUsed),
  )
  const [isRefreshing, setIsRefreshing] = useState<boolean>(initialDeals.length === 0)
  const [searchTerm, setSearchTerm] = useState("")
  const [priceFilter, setPriceFilter] = useState<[number, number] | null>(null)
  const [selectedAirlines, setSelectedAirlines] = useState<string[]>([])
  const [selectedDestinations, setSelectedDestinations] = useState<string[]>([])

  const fetchLatestDeals = useCallback(async () => {
    try {
      setIsRefreshing(true)
      setError(null)

      const response = await fetch("/api/deals", { cache: "no-store" })
      let payload: DealsApiResponse | null = null

      try {
        payload = (await response.json()) as DealsApiResponse
      } catch (parseError) {
        payload = null
      }

      if (!response.ok || !payload) {
        const message = payload?.error || "Deals konden niet worden geladen."
        throw new Error(message)
      }

      if (!Array.isArray(payload.items)) {
        throw new Error("Ongeldig antwoord van de deals API")
      }
  ...
  useEffect(() => {
    if (initialDeals.length === 0 || Boolean(fallbackUsed)) {
      fetchLatestDeals()
    }
  }, [initialDeals.length, fallbackUsed, fetchLatestDeals])
          ...
          <div className="text-center mb-8 space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              >
                ...
                {isRefreshing ? "Deals verversen..." : "Ververs deals"}
              </Button>
            </div>
          </div>

          {deals.length === 0 && isRefreshing ? (
